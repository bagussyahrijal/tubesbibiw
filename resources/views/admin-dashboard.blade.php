...
<body>
        ...
        <main class="main-content">
            ...
            <div class="dashboard-cards">
                <div class="dashboard-card">
                    ...
                    <div class="card-value" id="totalRevenueValue">Rp.1.500.000,00</div>
                    </div>
                ...
                <div class="dashboard-card">
                    ...
                    <div class="card-value" id="totalOrdersValue">527</div>
                    </div>
                ...
                <div class="dashboard-card">
                    ...
                    <div class="card-value" id="activeUsersValue">102</div>
                    </div>
            </div>
                    ...
                    <tbody id="recentOrdersTableBody">
                        <tr>
                            ...
                            <td>
                                ...
                                <button class="action-btn" title="Update Status"><i class="fas fa-edit"></i></button>
                            </td>
                        </tr>
                        <tr>
                            <td>#LC-78944</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const orderStatusRawData = [
                { status: 'Unpaid', count: 50, color: '#7f8c8d' },      // var(--unpaid-text-color)
                { status: 'Pending', count: 150, color: '#e67e22' },    // var(--warning-color)
                { status: 'Processing', count: 80, color: '#3498db' },  // var(--primary-color)
                { status: 'Completed', count: 2520, color: '#2ecc71' }  // var(--success-color)
            ];
            const orderStatusChartData = {
                labels: orderStatusRawData.map(item => item.status),
                datasets: [{
                    label: 'Order Status',
                    data: orderStatusRawData.map(item => item.count),
                    backgroundColor: orderStatusRawData.map(item => item.color),
                    hoverOffset: 8,
                    borderColor: '#fff',
                    borderWidth: 2
                }]
                        ...
                        options: {
                            ...
                            onHover: (event, chartElement) => {
                                ...
                                if (chartElement.length && orderStatusDetailValue) {
                                    ...
                                    orderStatusDetailValue.textContent = `${orderStatusChartData.labels[index]}: ${orderStatusChartData.datasets[0].data[index]}`;
                                    canvas.style.cursor = 'pointer';
                                } else if (orderStatusDetailValue) {
                                    orderStatusDetailValue.textContent = '- Hover on chart for details -';
                    ...
                    new Chart(ctxDailyRevenue.getContext('2d'), {
                        ...
                        options: {
                            ...
                            plugins: {
                                ...
                                tooltip: { callbacks: { label: context => `Revenue: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(context.parsed.y)}` } }
                            }
                        }
                    });
                }
